...
export interface FisherOptions<T> {
  ...
  setLocal(value: Fishable<T>): void;
}
...
export class Fisher<T> {
  ...
  async store(value: Fishable<T>): Promise<void> {
    const { cookieName, event, secretKeyBase64, setLocal } = this.opts;

    setLocal(value);

    const jwe = await encrypt(JSON.stringify(value), base64ToBytes(secretKeyBase64));
